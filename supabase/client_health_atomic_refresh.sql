...
begin
 ...
 if c->>'configStatus'='configuration_required' then
  ...
  if has_cur then select coalesce(sum((x->>'spend')::numeric),0),coalesce(sum((x->>'results')::numeric),0) into cur_spend,cur_results from public.client_health_source_runs s cross join lateral jsonb_array_elements(s.facts->'currentRows')x where s.refresh_run_id=r.id and s.client_id=p_client_id and s.run_status='succeeded' and jsonb_typeof(s.facts->'currentRows')='array';perform public.client_health_binary64_json(cur_spend,'current spend sum');perform public.client_health_binary64_json(cur_results,'current results sum');cur_cpr:=case when cur_results=0 then null else cur_spend/cur_results end;end if;
  if has_prev then select coalesce(sum((x->>'spend')::numeric),0),coalesce(sum((x->>'results')::numeric),0) into prev_spend,prev_results from public.client_health_source_runs s cross join lateral jsonb_array_elements(s.facts->'previousRows')x where s.refresh_run_id=r.id and s.client_id=p_client_id and s.run_status='succeeded' and jsonb_typeof(s.facts->'previousRows')='array';perform public.client_health_binary64_json(prev_spend,'previous spend sum');perform public.client_health_binary64_json(prev_results,'previous results sum');prev_cpr:=case when prev_results=0 then null else prev_spend/prev_results end;end if;
 ...
 snap:=jsonb_build_object('refreshRunId',r.id::text,'clientId',p_client_id::text,'snapshotDate',r.snapshot_date::text,'dataThrough',data_through,'budget',public.client_health_binary64_json(budget,'budget',true),'monthSpend',public.client_health_binary64_json(month_spend,'monthSpend',true),'expectedSpend',public.client_health_binary64_json(expected,'expectedSpend'),'currentWindowStart',(r.snapshot_date-13)::text,'currentWindowEnd',r.snapshot_date::text,'currentSpend',public.client_health_binary64_json(cur_spend,'currentSpend'),'currentResultCount',public.client_health_binary64_json(cur_results,'currentResultCount'),'currentCostPerResult',public.client_health_binary64_json(cur_cpr,'currentCostPerResult'),'previousWindowStart',(r.snapshot_date-27)::text,'previousWindowEnd',(r.snapshot_date-14)::text,'previousSpend',public.client_health_binary64_json(prev_spend,'previousSpend'),'previousResultCount',public.client_health_binary64_json(prev_results,'previousResultCount'),'previousCostPerResult',public.client_health_binary64_json(prev_cpr,'previousCostPerResult'),'hoursUsed',public.client_health_binary64_json(hours_used,'hoursUsed',true),'hoursAllotted',public.client_health_binary64_json(hours_allotted,'hoursAllotted',true),'projectedHours',public.client_health_binary64_json(projected,'projectedHours'),'overdueTaskCount',public.client_health_binary64_json(overdue,'overdueTaskCount',true),'revenue',public.client_health_binary64_json(revenue,'revenue',true),'fulfillmentCost',public.client_health_binary64_json(cost,'fulfillmentCost',true),'marginPercent',public.client_health_binary64_json(margin,'marginPercent'),'dimensionStatuses',ds,'sourceStatuses',coalesce(ss,'{}'),'overallStatus',overall,'overallScore',public.client_health_binary64_json(score,'overallScore'),'reasons',reasons,'calculatedAt',to_char(authoritative_at at time zone'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'));
