...
- After refresh creation, each process generates a cryptographically random per-execution claim-attempt UUID and atomically acquires an exclusive lease by sending only the refresh ID, logical invocation ID, claim-attempt ID, and `leaseDurationMs`. The lifecycle repository/database must atomically derive canonical `leaseGrantedAt` and `leaseExpiresAt` from its own commit-time clock; callers never supply an absolute expiry. The claim-attempt UUID and both lease timestamps are excluded from refresh identity and evidence. A logical invocation UUID, exact claim-attempt UUID, and positive monotonic fencing token are passed to every subsequent lifecycle mutation and atomic persistence call; implementations must reject inactive, expired, wrong-attempt, or stale-fence operations.
- Every grant and readback must return the exact refresh/invocation/attempt/fence plus canonical grant/expiry timestamps where `leaseExpiresAt - leaseGrantedAt === leaseDurationMs` and expiry is after grant. Lease acquisition response loss is reconciled only against that exact current committed state. A duplicate process using the same logical invocation still has a different attempt UUID and cannot adopt another process's lease. An invocation that cannot prove ownership performs no collection, persistence, cleanup, failure, validation, or publication against the shared run.
- Before every bounded owned phase, the orchestrator atomically renews the exact invocation-attempt-fence claim by duration. Renewal must retain the attempt and fence and strictly advance both authoritative `leaseGrantedAt` and `leaseExpiresAt`; response loss requires an exact current-state readback satisfying the same invariants. Uncertain renewal ownership stops every further mutation.
- `leaseDurationMs` is independently capped at 10 minutes and must be at least `4 * deadlineMs + 1000` using safe-integer arithmetic. This conservative lifetime covers the complete worst-case sequence of a renewal attempt, renewal reconciliation, the subsequent owned operation, and that operation's response-loss reconciliation, with a final one-second margin; checking only one operation deadline is insufficient.
