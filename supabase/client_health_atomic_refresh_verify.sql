...
begin
  ...
  if v_snapshot_json->>'currentWindowStart'<>'2026-08-07' or v_snapshot_json->>'previousWindowStart'<>'2026-07-24'
     or (v_snapshot_json->>'expectedSpend')::double precision<>64.51612903225806
     or (v_snapshot_json->>'marginPercent')::double precision<>75
     or v_snapshot_json->>'overallStatus'<>'at_risk' or (v_snapshot_json->>'overallScore')::double precision<>60
     ...
     or v_snapshot_json->'dimensionStatuses'<>jsonb_build_object(
       'budget_pacing',jsonb_build_object('status','watch','value',14.516129032258064,'reason','Budget pacing is 14.52 (watch).','required',true,'weight',20),
       'north_star',jsonb_build_object('status','at_risk','value',-44.44444444444444,'reason','North-star trend is -44.44 (at risk).','required',true,'weight',20),
       'hours',jsonb_build_object('status','healthy','value',31,'reason','Hours utilization is 31 (healthy).','required',true,'weight',20),
       'overdue_tasks',jsonb_build_object('status','watch','value',1,'reason','Overdue tasks is 1 (watch).','required',true,'weight',20),
       'margin',jsonb_build_object('status','healthy','value',75,'reason','Margin is 75 (healthy).','required',true,'weight',20)) then
