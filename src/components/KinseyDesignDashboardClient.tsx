...
function WeeklyExecutiveSummary({ readout }: { readout: KinseyDashboardData['weeklyReadout'] }) {
    ...
    <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-gray-900">Kinsey Design</h2>
...
function BudgetPacing({
  ...
  pacing: KinseyDashboardData['budgetPacing'];
  isAdmin: boolean;
  updateBudget: (n: number) => Promise<{ error?: string }>;
}) {
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      ...
      <ResponsiveContainer width="100%" height={240}>
        <AreaChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="knSpendGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor="#EB541E" stopOpacity={0.15} />
              <stop offset="95%" stopColor="#EB541E" stopOpacity={0} />
            </linearGradient>
            <linearGradient id="knMetricGrad" x1="0" y1="0" x2="0" y2="1">
          ...
          <Area yAxisId="spend" type="monotone" dataKey="Spend" stroke="#EB541E" strokeWidth={2} fill="url(#knSpendGrad)" dot={false} />
          <Area yAxisId="metric" type="monotone" dataKey={activeLabel} stroke="#0B4A31" strokeWidth={2} fill="url(#knMetricGrad)" dot={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
function CampaignTable({ rows }: { rows: KinseyDashboardData['campaignRows'] }) {
  ...
  const cols: { key: CampSortKey; label: string; fmt: (v: number) => string; prevKey: keyof KinseyDashboardData['campaignRows'][0]; invert?: boolean }[] = [
    { key: 'impressions', label: 'Impr.',   fmt: fmtN,    prevKey: 'prevImpressions' },
    { key: 'clicks',      label: 'Clicks',  fmt: fmtN,    prevKey: 'prevClicks' },
...
function AdPerformanceTable({ rows }: { rows: KinseyDashboardData['adRows'] }) {
  ...
  const numCols: { key: AdSortKey; label: string; fmt: (v: number) => string; prevKey: keyof KinseyDashboardData['adRows'][0] }[] = [
    { key: 'spend',     label: 'Spend',   fmt: fmt$,    prevKey: 'prevSpend' },
    { key: 'purchases', label: 'Sales',   fmt: fmtN,    prevKey: 'prevPurchases' },
    { key: 'revenue',   label: 'Revenue', fmt: fmt$,    prevKey: 'prevRevenue' },
    { key: 'roas',      label: 'ROAS',    fmt: fmtRoas, prevKey: 'prevRoas' },
...
export default function KinseyDesignDashboardClient({
  data,
  isAdmin,
  updateBudget,
}: {
  data: KinseyDashboardData;
  isAdmin: boolean;
  updateBudget: (n: number) => Promise<{ error?: string }>;
}) {
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-5">
        <div className="max-w-7xl mx-auto flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-gray-900">Kinsey Design</h1>
