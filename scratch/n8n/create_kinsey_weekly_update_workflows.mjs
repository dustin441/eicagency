...
const clickupWorkflowName = 'Kinsey Design ClickUp Sync';
const weeklyWorkflowName = 'Kinsey Design Weekly Readout Generator';
const kinseyListId = '901414385622';
const ddl = fs.readFileSync('scratch/kinsey/kinsey_weekly_update_tables.sql', 'utf8');
...
console.log(`Kinsey Design ClickUp Sync URL: ${baseUrl}/workflow/${savedClickup.id}`);
console.log(`Kinsey Design Weekly Readout Generator URL: ${baseUrl}/workflow/${savedWeekly.id}`);
...
function buildClickupWorkflow(template) {
  ...
  getAllTasks.parameters.url = `https://api.clickup.com/api/v2/list/${kinseyListId}/task`;
  ...
  const ensureTables = {
    id: 'kinsey-weekly-ensure-tables',
    name: 'Ensure Kinsey Weekly Tables',
  ...
  const manualTrigger = {
    id: 'kinsey-clickup-manual-trigger',
    name: 'Manual Trigger',
  ...
  workflow.nodes = workflow.nodes
    .filter((node) => !['Ensure Kinsey Weekly Tables', 'Manual Trigger'].includes(node.name))
...
function buildWeeklyWorkflow(template) {
  ...
  getComments.parameters.query = "SELECT clickup_comment_id AS comment_id, clickup_task_id AS task_id, comment_text, posted_at::text AS comment_date, commenter AS user_name, funnel_focus FROM kinsey_clickup_comments WHERE posted_at >= $1::date AND posted_at < ($2::date + interval '1 day') ORDER BY posted_at ASC LIMIT 300";
  getComments.parameters.options = {
    queryReplacement: '={{ $json.comments_start }},{{ $json.comments_end }}',
  };
  getTasks.parameters.query = 'SELECT clickup_task_id AS task_id, name AS task_name, funnel_focus FROM kinsey_clickup_tasks';
  ...
  schema.parameters.jsonSchemaExample = JSON.stringify({
    overall_story: 'Kinsey Design saw stronger purchase efficiency while traffic quality and creative testing shaped the next optimization priorities.',
  ...
  const ensureTables = {
    id: 'kinsey-weekly-readout-ensure-tables',
    name: 'Ensure Kinsey Weekly Tables',
    type: 'n8n-nodes-base.postgres',
  ...
  workflow.nodes = workflow.nodes
    .filter((node) => node.name !== 'Ensure Kinsey Weekly Tables')
    .concat([ensureTables]);
...
const query = "INSERT INTO public.kinsey_clickup_tasks (clickup_task_id, list_id, name, status, url, assignees, tags, funnel_focus, due_at, created_at, updated_at, synced_at, raw_payload) VALUES " + values +
...
")").join(', ');
const query = "INSERT INTO public.kinsey_clickup_comments (clickup_comment_id, clickup_task_id, list_id, comment_text, commenter, funnel_focus, posted_at, synced_at, raw_payload) VALUES " + values +
...
const performanceText = lines.length ? lines.join('\\n') : 'No Kinsey Design performance rows found for the selected period.';
...
const chatInput =
  ...
  'IMPORTANT CONTEXT: Treat Kinsey Design and Kinsey Designs as the same client context. Use ecommerce funnel framing where useful: Creative for ad assets and messaging, Traffic for click/CTR/CPC health, Conversion for purchases/conversions, revenue, ROAS, and cost per purchase/conversion, and Measurement for tracking or analytics issues.';
...
function weeklySystemMessage() {
  return `You are a senior paid media strategist at EIC Agency writing a weekly dashboard readout for Kinsey Design.
...
Hard rules:
- Treat Kinsey Design and Kinsey Designs as the same client context.
- Call out Meta versus Google when the data supports it.
- Include ecommerce funnel context in the summary or bullets where useful: Creative, Traffic, Conversion, and Measurement.
...
- Keep the tone professional, concise, and client-ready. This goes directly into the Kinsey Design dashboard.
...
const sql =
  "INSERT INTO kinsey_weekly_readout (week_of, period_start, period_end, previous_start, previous_end, overall_story, wins, opportunities, accomplishments, focus_next_week, execution_context, status, raw_agent_output) VALUES (" +
