...
const clickupWorkflowName = 'Arabella ClickUp Sync';
const weeklyWorkflowName = 'Arabella Weekly Readout Generator';
const arabellaListId = '901414345904';
const ddl = fs.readFileSync('scratch/arabella/arabella_weekly_update_tables.sql', 'utf8');
...
console.log(`Arabella ClickUp Sync URL: ${baseUrl}/workflow/${savedClickup.id}`);
console.log(`Arabella Weekly Readout Generator URL: ${baseUrl}/workflow/${savedWeekly.id}`);
...
function buildClickupWorkflow(template) {
  ...
  getAllTasks.parameters.url = `https://api.clickup.com/api/v2/list/${arabellaListId}/task`;
  ...
  const ensureTables = {
    id: 'arabella-weekly-ensure-tables',
    name: 'Ensure Arabella Weekly Tables',
  ...
  const manualTrigger = {
    id: 'arabella-clickup-manual-trigger',
    name: 'Manual Trigger',
  ...
  workflow.nodes = workflow.nodes
    .filter((node) => !['Ensure Arabella Weekly Tables', 'Manual Trigger'].includes(node.name))
...
function buildWeeklyWorkflow(template) {
  ...
  getComments.parameters.query = "SELECT clickup_comment_id AS comment_id, clickup_task_id AS task_id, comment_text, posted_at::text AS comment_date, commenter AS user_name, funnel_focus FROM arabella_clickup_comments WHERE posted_at >= $1::date AND posted_at < ($2::date + interval '1 day') ORDER BY posted_at ASC LIMIT 300";
  getComments.parameters.options = {
    queryReplacement: '={{ $json.comments_start }},{{ $json.comments_end }}',
  };
  getTasks.parameters.query = 'SELECT clickup_task_id AS task_id, name AS task_name, funnel_focus FROM arabella_clickup_tasks';
  ...
  schema.parameters.jsonSchemaExample = JSON.stringify({
    overall_story: 'Arabella Hotels improved booking volume and revenue efficiency on Meta, with ROAS holding above target while creative testing continued to shape the next optimization cycle.',
    wins: ['Meta produced stronger booking volume and revenue during the period.'],
    opportunities: ['Creative and landing-page efficiency should be reviewed where clicks increased without a matching lift in bookings.'],
    accomplishments: ['Reviewed weekly budget pacing and optimization comments from ClickUp.'],
    focus_next_week: ['Prioritize creative tests and budget shifts around the highest-ROAS campaign and ad pockets.'],
    execution_context: ['ClickUp comments may include general budget pacing notes; only attribute them to Creative, Traffic, Booking, or Measurement when the text supports it.'],
  });
  formatSaveData.parameters.jsCode = weeklyFormatSaveCode();

  const ensureTables = {
    id: 'arabella-weekly-readout-ensure-tables',
    name: 'Ensure Arabella Weekly Tables',
    type: 'n8n-nodes-base.postgres',
  ...
  workflow.nodes = workflow.nodes
    .filter((node) => node.name !== 'Ensure Arabella Weekly Tables')
    .concat([ensureTables]);
...
function focusInferenceHelperCode() {
  ...
  if (text.includes('booking') || text.includes('purchase') || text.includes('roas') || text.includes('revenue') || text.includes('sale')) matches.push('booking');
...
const query = "INSERT INTO public.arabella_clickup_tasks (clickup_task_id, list_id, name, status, url, assignees, tags, funnel_focus, due_at, created_at, updated_at, synced_at, raw_payload) VALUES " + values +
...
")").join(', ');
const query = "INSERT INTO public.arabella_clickup_comments (clickup_comment_id, clickup_task_id, list_id, comment_text, commenter, funnel_focus, posted_at, synced_at, raw_payload) VALUES " + values +
...
for (const channel of channels) {
  ...
  const prevCpp = prevPurchases > 0 ? prevSpend / prevPurchases : null;
  lines.push([
    channel,
    'Spend ' + money(spend) + ' (' + pct(spend, prevSpend) + ')',
    'Bookings/Purchases ' + purchases + ' (' + pct(purchases, prevPurchases) + ')',
    'Cost per Purchase ' + (cpp == null ? 'N/A' : money(cpp)) + ' (' + (cpp != null && prevCpp != null ? pct(cpp, prevCpp) : 'N/A') + ')',
...
const performanceText = lines.length ? lines.join('\\n') : 'No Arabella performance rows found for the selected period.';
...
const chatInput =
  ...
  'IMPORTANT CONTEXT: Treat Arabella and Arabella Hotels as the same client context. Arabella is currently Meta-focused in the dashboard. Use hotel funnel framing where useful: Creative for ad assets and messaging, Traffic for click/CTR/CPC health, Booking for purchases, revenue, ROAS, and cost per purchase, and Measurement for tracking or analytics issues.';
...
function weeklySystemMessage() {
  return `You are a senior paid media strategist at EIC Agency writing a weekly dashboard readout for Arabella Hotels.
...
- opportunities: Optimization opportunities, especially budget pacing, creative testing, traffic quality, booking efficiency, revenue efficiency, or measurement cleanup.
- accomplishments: Concrete actions or changes from ClickUp comments.
- focus_next_week: Specific priorities for next week.
- execution_context: Context that affects interpretation, such as budget pacing notes, measurement gaps, limited booking volume, or comments being general.

Hard rules:
- Treat Arabella and Arabella Hotels as the same client context.
- Call out Meta as the primary active channel unless the data explicitly includes another channel.
- Include hotel funnel context in the summary or bullets where useful: Creative, Traffic, Booking, and Measurement.
- Prioritize purchases/bookings, revenue, ROAS, and cost per purchase when booking data exists.
- Use ClickUp comments as operational context. Do not invent accomplishments that are not supported by comments.
- Keep the tone professional, concise, and client-ready. This goes directly into the Arabella dashboard.
...
const sql =
  "INSERT INTO arabella_weekly_readout (week_of, period_start, period_end, previous_start, previous_end, overall_story, wins, opportunities, accomplishments, focus_next_week, execution_context, status, raw_agent_output) VALUES (" +
