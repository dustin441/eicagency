...
assert.match(render(undefined), /no published forward cohort overlaps the selected dates/);
...
const input = {
  ...
  contacts: [{ contactId: 'PRIVATE_SYNTHETIC_ID', quizEntryAt: '2026-09-01T00:00:00Z', acquisition: 'paid', attributionSnapshot: { normalizedSource: 'instagram', normalizedChannel: 'paid_social', schemaVersion: 'historical-v2', snapshotAt: '2026-09-02T00:00:00Z', conflict: false } }], events: [],
...
for (const label of ['Quiz lead', 'Appointment scheduled', 'Closer scheduled', 'Closed won', 'Optimization KPI', 'start inclusive, end exclusive', 'Observed through (inclusive)', 'Verified Meta-paid only']) assert.ok(complete.includes(label), label);
...
assert.equal((missing.match(/<strong>Unavailable<\/strong>/g) ?? []).length, 2);
assert.match(missing, /not complete outcome totals/);
console.log('IHH isolated cohort panel SSR checks passed: blocked/error, four stages, Meta-only scope/KPI, zero/missing, no filter UI, no contact IDs or private provenance.');
