...
async function hasNsiAccess() {
  ...
  return {
    ok: Boolean(profile.client_access?.includes('nsi')),
    status: 403,
  };
}

function getSafeDashboardUrl(request: NextRequest) {
  const path = request.nextUrl.searchParams.get('path') ?? '/dashboard/nsi';
  if (path !== '/dashboard/nsi' && !path.startsWith('/dashboard/nsi/')) {
    return null;
  }
...
function filenameForPath(pathname: string) {
  ...
  const segment = last === 'nsi' ? 'dashboard' : last;
  const today = new Date().toISOString().slice(0, 10);
  return `nsi-${segment}-${today}.pdf`;
}
...
export async function GET(request: NextRequest) {
  const access = await hasNsiAccess();
  if (!access.ok) {
  ...
  try {
    ...
    console.error('NSI PDF export failed', error);
