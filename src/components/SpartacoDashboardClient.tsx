...
export default function SpartacoDashboardClient({ data }: { data: SpartacoDashboardData }) {
  ...
  const coreKpis = [
    ...
    },
    {
      ...
      value: isAll ? fmtCurrency(data.summary.cost) : metricValue(data.mode, 'value', data.summary),
      delta: isAll ? pctChange(data.summary.cost, data.previousSummary.cost) : metricDelta(data.mode, 'value', data.summary, data.previousSummary).text,
      current: isAll ? data.summary.cost : metricDelta(data.mode, 'value', data.summary, data.previousSummary).current,
      previous: isAll ? data.previousSummary.cost : metricDelta(data.mode, 'value', data.summary, data.previousSummary).previous,
      lowerIsBetter: true,
      icon: DollarSign,
