...
function TrendChart({ timeSeries }: { timeSeries: TurfliDashboardData['timeSeries'] }) {
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      ...
      <ResponsiveContainer width="100%" height={240}>
        <AreaChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
          ...
          <Area yAxisId="spend" type="monotone" dataKey="Spend" stroke="#EB541E" strokeWidth={2} fill="url(#spendGrad)" dot={false} />
          <Area yAxisId="metric" type="monotone" dataKey={activeMetric.charAt(0).toUpperCase() + activeMetric.slice(1)} stroke="#0B4A31" strokeWidth={2} fill="url(#metricGrad)" dot={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
function CampaignTable({ rows }: { rows: TurfliDashboardData['campaignRows'] }) {
...
function BudgetPacing({
  ...
  pacing: TurfliDashboardData['budgetPacing'];
  isAdmin: boolean;
  updateBudget: (n: number) => Promise<{ error?: string }>;
}) {
...
export default function TurfliDashboardClient({
  data,
  isAdmin,
  updateBudget,
}: {
  data: TurfliDashboardData;
  isAdmin: boolean;
  updateBudget: (n: number) => Promise<{ error?: string }>;
}) {
    ...
    <div className="min-h-screen bg-gray-50/50">
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-5">
        <div className="max-w-7xl mx-auto flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-gray-900">Turfli</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <KpiCard label="Cost / Conv." value={summary.costPerConversion} prev={prevSummary.costPerConversion} format={fmt$} invert goal={75} goalFmt={fmt$} />
        </div>
