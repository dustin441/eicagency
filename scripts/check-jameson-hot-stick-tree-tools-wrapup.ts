...
async function main() {
  ...
  assert.deepEqual(wrapup.config.sourceMediumPagePaths, ['/lp/hot-stick-tools']);
  ...
  assert.equal(before.ga4_sessions, 72);
  assert.equal(before.ga4_engaged_sessions, 29);
  ...
  assert.equal(during.ga4_sessions, 851);
  assert.equal(during.ga4_engaged_sessions, 379);
  ...
  assert.equal(after.ga4_sessions, 34);
  assert.equal(after.ga4_engaged_sessions, 26);
  ...
  assert.equal(wrapup.outcomeAttribution.totalSessions, 851);
  assert.equal(wrapup.outcomeAttribution.totalEngagedSessions, 379);
  ...
  assert.ok(
    wrapup.config.executiveSummary.includes('separate from the later/current HERO Hot Stick campaign')
      && wrapup.config.executiveSummary.includes('851 campaign-period sessions')
      && wrapup.config.executiveSummary.includes('campaign page only'),
    'Expected executive summary to preserve non-HERO Hot-Stick campaign-page framing'
  );
  assert.ok(
