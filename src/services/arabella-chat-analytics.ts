...
export type ArabellaeSummaryRow = {
  campaignType: string;
  spend: number;
  impressions: number;
  clicks: number;
...
export type ArabellaeCampaignRow = {
  campaign: string;
  campaignType: string;
  spend: number;
  impressions: number;
...
export async function fetchArabellaeChatSummary(
  startDate?: string,
  endDate?: string,
  days?: number,
): Promise<ArabellaeSummaryRow[]> {
  const supabase = createSpartacoSupabaseClient();
  const { start, end } = resolveDateRange(startDate, endDate, days);

  const { data, error } = await supabase.rpc('arabella_chat_summary', {
    p_start: start, p_end: end,
  });
  if (error) throw new Error(error.message);

  const rows = (data ?? []) as unknown as {
    campaign_type: string; spend: number; impressions: number; clicks: number;
...
export async function fetchArabellaeChatCampaigns(
  startDate?: string,
  endDate?: string,
  days?: number,
  limit = 20,
): Promise<ArabellaeCampaignRow[]> {
  const supabase = createSpartacoSupabaseClient();
  const { start, end } = resolveDateRange(startDate, endDate, days);

  const { data, error } = await supabase.rpc('arabella_chat_campaigns', {
...
export async function fetchArabellaeChatSpendTrend(
  startDate?: string,
  endDate?: string,
  days?: number,
  ...
  const { data, error } = await supabase.rpc('arabella_chat_spend_trend', {
    p_start: start, p_end: end,
  });
  if (error) throw new Error(error.message);

  const rows = (data ?? []) as unknown as {
    date_day: string; spend: number; purchases: number;
    revenue: number; clicks: number; impressions: number;
  }[];

  // purchases → won for chart compatibility
  return {
    focus: 'All Campaigns',
    platform: 'Meta',
    startDate: start,
...
export async function fetchArabellaeChatMetaCreatives(
  startDate?: string,
  endDate?: string,
  days?: number,
  limit = 10,
  ...
  const { data, error } = await supabase.rpc('arabella_chat_meta_creatives', {
    p_start: start, p_end: end, p_limit: limit,
  });
  if (error) throw new Error(error.message);
