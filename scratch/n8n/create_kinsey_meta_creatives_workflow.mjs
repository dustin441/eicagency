...
const kinseyCredentialWorkflowId = 'JowOQLH0aenhf28O';
const kinseyAdAccountId = '243586875018967';
const targetWorkflowName = 'Meta Ads Puller Creatives to Supabase - Kinsey Design ✅';
...
const [prepassWorkflow, kinseyWorkflow] = await Promise.all([
  fetchJson(`${baseUrl}/api/v1/workflows/${prepassWorkflowId}`),
  fetchJson(`${baseUrl}/api/v1/workflows/${kinseyCredentialWorkflowId}`),
]);

const kinseyNodes = new Map(kinseyWorkflow.nodes.map((node) => [node.name, node]));
const kinseyPullData = requireNode(kinseyNodes, 'Pull Data');
const kinseyUpsert = requireNode(kinseyNodes, 'Insert or update rows in a table');
...
workflow.nodes = workflow.nodes.map((node) => {
  ...
  if (next.type === 'n8n-nodes-base.httpRequest') {
    next.credentials = structuredClone(kinseyPullData.credentials);
  }

  if (next.type === 'n8n-nodes-base.postgres') {
    next.credentials = structuredClone(kinseyUpsert.credentials);
  }

  if (next.name === 'Pull Data') {
    next.parameters.url = `https://graph.facebook.com/v19.0/act_${kinseyAdAccountId}/insights`;
  ...
  if (next.name === 'Fetch Image URLs') {
    next.parameters.url = `=https://graph.facebook.com/v19.0/act_${kinseyAdAccountId}/adimages?hashes[]={{ $json.image_hash }}&fields=hash,url`;
  }
  ...
  if (next.name === 'Insert or update rows in a table') {
    next.parameters.table = {
      __rl: true,
      mode: 'name',
      value: 'kinsey_meta_ads_creatives',
    };
...
workflow.nodes.push(
  {
    ...
    credentials: structuredClone(kinseyUpsert.credentials),
  },
  {
    parameters: {
...
console.log(`Kinsey creative workflow URL: ${baseUrl}/workflow/${saved.id}`);
...
function alterTableSql() {
  return `CREATE TABLE IF NOT EXISTS public.kinsey_meta_ads_creatives (
...
ALTER TABLE public.kinsey_meta_ads_creatives
