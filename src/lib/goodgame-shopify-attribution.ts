export type GoodGameShopifyAttributionDailyRow = {
  ...
  shopify_first_order_revenue: number;
  shopify_lifetime_net_revenue: number;
...
export function summariseGoodGameShopifyAttribution(
  ...
  const attributedSpend = rows.reduce((sum, row) => sum + Number(row.media_spend ?? 0), 0);
  const firstOrderRevenue = rows.reduce((sum, row) => sum + Number(row.shopify_first_order_revenue ?? 0), 0);
  const lifetimeNetRevenue = rows.reduce((sum, row) => sum + Number(row.shopify_lifetime_net_revenue ?? 0), 0);
  ...
  const averageLtv = newCustomers > 0 ? lifetimeNetRevenue / newCustomers : 0;
...
export function aggregateGoodGameShopifyCampaigns(
  ...
  for (const row of rows) {
    ...
    entry.firstOrderRevenue += Number(row.shopify_first_order_revenue ?? 0);
    entry.lifetimeNetRevenue += Number(row.shopify_lifetime_net_revenue ?? 0);
  ...
  return Array.from(map.values())
    .map((row) => {
      ...
      const averageLtv = row.newCustomers > 0 ? row.lifetimeNetRevenue / row.newCustomers : 0;
      return { ...row, cac, averageLtv, ltvCac: cac > 0 ? averageLtv / cac : 0 };
    })
