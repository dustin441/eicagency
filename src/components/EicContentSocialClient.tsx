...
export default function EicContentSocialClient({ data, approvePost, rejectPost, updatePost, createUploadBatch }: Props) {
  ...
  function submitUpload() { const formData = new FormData(); formData.set('episode_title', uploadTitle || selectedEpisode?.title || `Podcast upload ${safeDateTitle()}`); formData.set('recording_date', uploadDate); formData.set('source_type', 'dashboard_inline_drop'); formData.set('notes', 'Uploaded from the main Content Hub editing page. Expected package: full episode, clips, and transcript.'); uploadFiles.forEach((file) => formData.append('files', file)); startTransition(async () => { const result = await createUploadBatch(formData); setUploadMessage({ ok: result.ok, text: result.message }); if (result.ok) { setUploadFiles([]); setUploadTitle(''); setUploadDate(''); } }); }
