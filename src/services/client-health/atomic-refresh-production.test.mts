...
test('RPC adapter maps every lifecycle and persistence method to exact names, arguments, ownership, and signals', async () => {
  const h = harness();
  const refresh = { id: '33333333-3333-4333-8333-333333333333', refreshIdentityHash: 'a'.repeat(64), runAttemptId: '77777777-7777-4777-8777-777777777777', snapshotDate: '2026-08-20', calculationVersion: 'v1', sourceContractVersion: 's1', startedAt: '2026-08-21T00:00:00.000Z' };
  ...
  const receipt = await h.adapter.createRefreshRun(refresh, { signal });
  ...
  assert.deepEqual(h.calls[0].args, { p_id: refresh.id, p_refresh_identity_hash: refresh.refreshIdentityHash, p_run_attempt_id: refresh.runAttemptId, p_snapshot_date: refresh.snapshotDate, p_calculation_version: 'v1', p_source_contract_version: 's1', p_started_at: refresh.startedAt });
  assert.deepEqual(h.calls[7].args, { p_id: source.id, p_invocation_id: owned.invocationId, p_claim_attempt_id: owned.claimAttemptId, p_fencing_token: 7 });
  for (const index of [3, 5, 6, 7, 8, 9, 10, 11, 12]) {
  ...
  assert.equal(h.calls[9].args.p_bundle, bundle);
});
