...
const clickupWorkflowName = 'LifeRep ClickUp Sync';
const weeklyWorkflowName = 'LifeRep Weekly Readout Generator';
const liferepListId = '901415432028';
const ddl = fs.readFileSync('scratch/liferep/liferep_weekly_update_tables.sql', 'utf8');
...
console.log(`LifeRep ClickUp Sync URL: ${baseUrl}/workflow/${savedClickup.id}`);
console.log(`LifeRep Weekly Readout Generator URL: ${baseUrl}/workflow/${savedWeekly.id}`);
...
function buildClickupWorkflow(template) {
  ...
  getAllTasks.parameters.url = `https://api.clickup.com/api/v2/list/${liferepListId}/task`;
  ...
  const ensureTables = {
    id: 'liferep-weekly-ensure-tables',
    name: 'Ensure LifeRep Weekly Tables',
  ...
  const manualTrigger = {
    id: 'liferep-clickup-manual-trigger',
    name: 'Manual Trigger',
  ...
  workflow.nodes = workflow.nodes
    .filter((node) => !['Ensure LifeRep Weekly Tables', 'Manual Trigger'].includes(node.name))
...
function buildWeeklyWorkflow(template) {
  ...
  getComments.parameters.query = "SELECT clickup_comment_id AS comment_id, clickup_task_id AS task_id, comment_text, posted_at::text AS comment_date, commenter AS user_name, funnel_focus FROM liferep_clickup_comments WHERE posted_at >= $1::date AND posted_at < ($2::date + interval '1 day') ORDER BY posted_at ASC LIMIT 300";
  getComments.parameters.options = {
    queryReplacement: '={{ $json.comments_start }},{{ $json.comments_end }}',
  };
  getTasks.parameters.query = 'SELECT clickup_task_id AS task_id, name AS task_name, funnel_focus FROM liferep_clickup_tasks';
  ...
  schema.parameters.jsonSchemaExample = JSON.stringify({
    overall_story: 'LifeRep saw stronger purchase efficiency while Meta creative and traffic quality shaped the next optimization priorities.',
    wins: ['Meta produced stronger purchase volume and ROAS during the period.'],
  ...
  const ensureTables = {
    id: 'liferep-weekly-readout-ensure-tables',
    name: 'Ensure LifeRep Weekly Tables',
    type: 'n8n-nodes-base.postgres',
  ...
  workflow.nodes = workflow.nodes
    .filter((node) => node.name !== 'Ensure LifeRep Weekly Tables')
    .concat([ensureTables]);
...
const query = "INSERT INTO public.liferep_clickup_tasks (clickup_task_id, list_id, name, status, url, assignees, tags, funnel_focus, due_at, created_at, updated_at, synced_at, raw_payload) VALUES " + values +
...
")").join(', ');
const query = "INSERT INTO public.liferep_clickup_comments (clickup_comment_id, clickup_task_id, list_id, comment_text, commenter, funnel_focus, posted_at, synced_at, raw_payload) VALUES " + values +
...
function weeklyPerformanceQuery() {
  ...
  SELECT
    ...
    COALESCE(purchases, 0)::numeric AS actions,
...
const channels = ['all', 'Meta', 'Unknown'];
const lines = [];
...
const performanceText = lines.length ? lines.join('\\n') : 'No LifeRep performance rows found for the selected period.';
...
const chatInput =
  ...
  'IMPORTANT CONTEXT: LifeRep is currently Meta-focused in the dashboard. Use ecommerce funnel framing where useful: Creative for ad assets and messaging, Traffic for click/CTR/CPC health, Conversion for purchases, revenue, ROAS, and cost per purchase, and Measurement for tracking or analytics issues.';
...
function weeklySystemMessage() {
  return `You are a senior paid media strategist at EIC Agency writing a weekly dashboard readout for LifeRep.
...
Hard rules:
- Treat LifeRep as the client context.
...
- Prioritize purchases, revenue, ROAS, and cost per purchase when conversion data exists.
- Use ClickUp comments as operational context. Do not invent accomplishments that are not supported by comments.
- Keep the tone professional, concise, and client-ready. This goes directly into the LifeRep dashboard.
...
const sql =
  "INSERT INTO liferep_weekly_readout (week_of, period_start, period_end, previous_start, previous_end, overall_story, wins, opportunities, accomplishments, focus_next_week, execution_context, status, raw_agent_output) VALUES (" +
