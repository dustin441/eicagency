...
export default function FleetSizeTable({
  ...
  const totalMqls = data.reduce((a, d) => a + (d.mqls ?? 0), 0);
  const totalSqls = data.reduce((a, d) => a + (d.sqls ?? 0), 0);
  const totalWon = data.reduce((a, d) => a + (d.won ?? 0), 0);
    ...
    <div className="w-full bg-white border border-gray-100 shadow-sm rounded-[2.5rem] overflow-hidden">
      <div className="p-8 border-b border-gray-100">
        ...
        <p className="text-sm text-gray-400 font-medium mt-0.5">
          {subtitle ?? `Leads, funnel & cost per lead by fleet size · ${fmtN(totalLeads)} attributed leads · MQL/SQL/WON = leads in band that reached each stage · Cost/Lead = total ABM investment ÷ leads in band`}
        </p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              {['Fleet Size', 'Leads', '% of Leads', 'MQL', 'SQL', 'WON', 'Cost/Lead'].map(h => (
          ...
          <tbody className="divide-y divide-gray-50">
            {data.map(d => (
              <tr key={d.band} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-6 py-4 tabular-nums font-medium">{fmtN(d.mqls ?? 0)}</td>
                <td className="px-6 py-4 tabular-nums font-medium">{fmtN(d.sqls ?? 0)}</td>
                <td className="px-6 py-4 tabular-nums font-semibold text-brand-forest">{fmtN(d.won ?? 0)}</td>
          ...
          <tfoot className="border-t border-gray-100 bg-gray-50">
            <tr>
              ...
              <td className="px-6 py-4 tabular-nums font-bold">{fmtN(totalMqls)}</td>
              <td className="px-6 py-4 tabular-nums font-bold">{fmtN(totalSqls)}</td>
              <td className="px-6 py-4 tabular-nums font-bold text-brand-forest">{fmtN(totalWon)}</td>
