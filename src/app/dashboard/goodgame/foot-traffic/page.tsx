...
import { updateGoodGameFootTrafficBudget } from '../actions';

export default async function GoodGameFootTrafficPage({
  searchParams,
}: {
  ...
  const data = await fetchGoodGameDashboardData(params, 'foot_traffic');

  return (
    <GoodGameDashboardClient
      data={data}
      isAdmin={isAdmin}
      updateBudget={updateGoodGameFootTrafficBudget}
    />
  );
}
