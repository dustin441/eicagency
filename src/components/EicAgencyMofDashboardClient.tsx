...
import type { EicAgencyDashboardData, EicAgencyTimePoint } from '@/services/eicagency-analytics';
...
const MOF_METRICS: { key: string; label: string; color: string; fmt: (v: number) => string }[] = [
  { key: 'leads',       label: 'Eng. 30s',        color: '#10B981', fmt: (v) => Math.round(v).toLocaleString() },
  ...
  { key: 'cpl',         label: 'Cost/Eng. 30s',    color: '#6366F1', fmt: (v) => `$${v.toFixed(2)}` },
];
...
function MofTrendChart({ data, start, end }: { data: EicAgencyTimePoint[]; start: string; end: string }) {
  ...
  const activeList = MOF_METRICS.filter(m => activeMetrics.has(m.key));
          ...
          <ComposedChart data={chartData}>
            ...
            <Tooltip
              ...
              formatter={(value, name) => {
                ...
                const m = MOF_METRICS.find(x => x.key === name);
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Eng. 30s</th>
              <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Cost/Eng. 30s</th>
            </tr>
          </thead>
...
export default function EicAgencyMofDashboardClient({
  data,
}: {
  data: EicAgencyDashboardData;
}) {
  const { summary, prevSummary, timeSeries, channelRows, campaignRows, adSetRows, metaCreatives, dataFreshness } = data;
  const hasLandingPageViews = campaignRows.some(r => r.landingPageViews > 0);

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">EIC Agency — Performance MOF</h1>
        <p className="text-sm text-gray-500 mt-1">MOF Campaigns · Meta</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <KpiCard label="Engagement 30s" value={summary.leads}       prev={prevSummary.leads}       format={fmtN} highlight />
        <KpiCard label="Cost/Eng. 30s"  value={summary.cpl}         prev={prevSummary.cpl}         format={fmt$2} invert />
      </div>

      {timeSeries.length > 1 && (
        <MofTrendChart data={timeSeries} start={data.filterParams.start} end={data.filterParams.end} />
      )}
      ...
      {metaCreatives.length > 0 && (
        <MetaAdPreviews
          ...
          description="Ad-level performance · MOF campaigns · Video ads open in Facebook Ad Library"
          advertiserName="EIC Agency"
          conversionLabel={{ conversion: 'Eng. 30s', cpa: 'Cost/Eng. 30s' }}
        />
      )}
    </div>
  );
