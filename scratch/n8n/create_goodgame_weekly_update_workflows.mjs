...
const clickupWorkflowName = 'Good Game ClickUp Sync';
const weeklyWorkflowName = 'Good Game Weekly Readout Generator';
const goodGameListId = '901414768821';
const ddl = fs.readFileSync('scratch/goodgame/goodgame_weekly_update_tables.sql', 'utf8');
...
console.log(`Good Game ClickUp Sync URL: ${baseUrl}/workflow/${savedClickup.id}`);
console.log(`Good Game Weekly Readout Generator URL: ${baseUrl}/workflow/${savedWeekly.id}`);
...
function buildClickupWorkflow(template) {
  ...
  getAllTasks.parameters.url = `https://api.clickup.com/api/v2/list/${goodGameListId}/task`;
  ...
  const ensureTables = {
    id: 'goodgame-weekly-ensure-tables',
    name: 'Ensure Good Game Weekly Tables',
  ...
  const manualTrigger = {
    id: 'goodgame-clickup-manual-trigger',
    name: 'Manual Trigger',
  ...
  workflow.nodes = workflow.nodes
    .filter((node) => !['Ensure Good Game Weekly Tables', 'Manual Trigger'].includes(node.name))
...
function buildWeeklyWorkflow(template) {
  ...
  getComments.parameters.query = "SELECT clickup_comment_id AS comment_id, clickup_task_id AS task_id, comment_text, posted_at::text AS comment_date, commenter AS user_name, funnel_focus FROM goodgame_clickup_comments WHERE posted_at >= $1::date AND posted_at < ($2::date + interval '1 day') ORDER BY posted_at ASC LIMIT 300";
  getComments.parameters.options = {
    queryReplacement: '={{ $json.comments_start }},{{ $json.comments_end }}',
  };
  getTasks.parameters.query = 'SELECT clickup_task_id AS task_id, name AS task_name, funnel_focus FROM goodgame_clickup_tasks';
  ...
  schema.parameters.jsonSchemaExample = JSON.stringify({
    overall_story: 'Good Game saw stronger purchase efficiency on Meta while Google held steady traffic volume, creating a clear next step to protect ROAS and sharpen creative testing.',
    wins: ['Meta produced the strongest purchase volume and ROAS during the period.'],
    ...
    focus_next_week: ['Prioritize creative tests and budget shifts around the highest-ROAS campaign pockets.'],
    execution_context: ['ClickUp comments may include general budget pacing notes; only attribute them to Engagement, Traffic, or Conversion when the text supports it.'],
  });
  formatSaveData.parameters.jsCode = weeklyFormatSaveCode();

  const ensureTables = {
    id: 'goodgame-weekly-readout-ensure-tables',
    name: 'Ensure Good Game Weekly Tables',
    type: 'n8n-nodes-base.postgres',
  ...
  workflow.nodes = workflow.nodes
    .filter((node) => node.name !== 'Ensure Good Game Weekly Tables')
    .concat([ensureTables]);
...
function focusInferenceHelperCode() {
  ...
  const matches = [];
  if (text.includes('engagement') || text.includes('video') || text.includes('view') || text.includes('thruplay')) matches.push('engagement');
  if (text.includes('traffic') || text.includes('click') || text.includes('cpc') || text.includes('ctr')) matches.push('traffic');
  if (text.includes('conversion') || text.includes('purchase') || text.includes('roas') || text.includes('revenue') || text.includes('sale')) matches.push('conversion');
...
const query = "INSERT INTO public.goodgame_clickup_tasks (clickup_task_id, list_id, name, status, url, assignees, tags, funnel_focus, due_at, created_at, updated_at, synced_at, raw_payload) VALUES " + values +
...
")").join(', ');
const query = "INSERT INTO public.goodgame_clickup_comments (clickup_comment_id, clickup_task_id, list_id, comment_text, commenter, funnel_focus, posted_at, synced_at, raw_payload) VALUES " + values +
...
for (const channel of channels) {
  ...
  lines.push([
    ...
    'Purchases/Conversions ' + purchases + ' (' + pct(purchases, prevPurchases) + ')',
    'Cost per Purchase/Conversion ' + (cpp == null ? 'N/A' : money(cpp)) + ' (' + (cpp != null && prevCpp != null ? pct(cpp, prevCpp) : 'N/A') + ')',
...
const performanceText = lines.length ? lines.join('\\n') : 'No Good Game performance rows found for the selected period.';
...
const chatInput =
  'CLIENT: Good Game - Nappy Boy Dranks\\n' +
  ...
  'IMPORTANT CONTEXT: Treat Good Game and Nappy Boy Dranks as the same client context. Use full-funnel framing where useful: Engagement for video/view quality, Traffic for click/CTR/CPC health, and Conversion for purchases, revenue, ROAS, and cost per purchase.';
...
function weeklySystemMessage() {
  return `You are a senior paid media strategist at EIC Agency writing a weekly dashboard readout for Good Game - Nappy Boy Dranks.
...
- execution_context: Context that affects interpretation, such as budget pacing notes, measurement gaps, limited purchase volume, or comments being general.

Hard rules:
- Treat Good Game and Nappy Boy Dranks as the same client context.
- Call out Meta versus Google when the data supports it.
- Include full-funnel context in the summary or bullets where useful: Engagement, Traffic, and Conversion.
- Prioritize purchases/conversions, revenue, ROAS, and cost per purchase/conversion when conversion data exists.
- Use ClickUp comments as operational context. Do not invent accomplishments that are not supported by comments.
- Keep the tone professional, concise, and client-ready. This goes directly into the Good Game dashboard.
...
const sql =
  "INSERT INTO goodgame_weekly_readout (week_of, period_start, period_end, previous_start, previous_end, overall_story, wins, opportunities, accomplishments, focus_next_week, execution_context, status, raw_agent_output) VALUES (" +
