...
assert.equal(
  ...
  'https://scontent.example/image.png?stp=dst-jpg_p160x120_q75',
  'a tiny image without a better candidate must remain visible rather than falling back to no preview',
);
assert.equal(
  resolveMetaImageUrl({
...
assert.match(component, /candidates=\{referenceCandidates \?\? candidates\}/, 'priority-test references must use same-view candidates when a dedicated insight-window set is unavailable');
