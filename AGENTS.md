...
- **Tiiger brand filter in `spartaco_master_products` requires an `.or()` clause with three conditions.** Tiiger data is split across: (1) direct `brand='Tiiger'` rows (~22), (2) `brand='Huskie'` / `product='Other'` ads rows remapped by campaign name via `remapOtherRow()`, and (3) `brand='Huskie'` / `product='Pole Puller'` GA4 rows (2,110+ rows) where the GA4 sync stores them under the wrong brand. Always use `.or('brand.eq.Tiiger,and(brand.eq.Huskie,product.eq.Other),and(brand.eq.Huskie,product.eq.Pole Puller)')` when `brandArg === 'Tiiger'`. `remapOtherRow()` also re-brands the Huskie/Pole Puller rows to Tiiger so they appear in the product dropdown. See `applyProductFilters()` in `spartaco-product-analytics.ts`.
