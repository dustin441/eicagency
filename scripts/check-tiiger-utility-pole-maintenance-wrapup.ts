...
async function main() {
  ...
  assert.equal(productData.summary.email_opens, 1392);
  ...
  assert.equal(during.email_opens, 1392);
  ...
  assert.equal(wrapup.emailDetails[0]?.opens, 1392);
  ...
  assert.equal(Math.round(wrapup.emailBenchmark.productOpenRate * 10000) / 100, 18.61);
