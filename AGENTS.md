...
- **Tiiger brand filter in `spartaco_master_products` requires an `.or()` clause with four conditions.** Tiiger data is split across: (1) direct `brand='Tiiger'` rows, (2) `brand='Huskie'` / `product='Other'` ads rows remapped by campaign name via `remapOtherRow()`, (3) `brand='Huskie'` / `product='Pole Puller'` GA4 rows where the GA4 sync stores them under the wrong brand, and (4) `brand='Huskie'` / `product='Pole Maintenance'` ad rows for Tiiger pole maintenance campaigns stored in the Huskie ad account. Always use `.or('brand.eq.Tiiger,and(brand.eq.Huskie,product.eq.Other),and(brand.eq.Huskie,product.eq.Pole Puller),and(brand.eq.Huskie,product.eq.Pole Maintenance)')` when `brandArg === 'Tiiger'`. `remapOtherRow()` re-brands all Huskie/Pole Puller and Huskie/Pole Maintenance rows to Tiiger. See `applyProductFilters()` in `spartaco-product-analytics.ts`.
