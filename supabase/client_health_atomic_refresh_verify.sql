...
declare
  ...
  v_revision jsonb := $revision${"schemaVersion":2,"calculationVersion":"verify-v2","sourceContractVersion":"verify-s2","clients":[{"clientId":"90000000-0000-4000-8000-000000000001","clientKey":"atomic_verify_fixture","displayName":"Atomic verify fixture","dashboardHref":"/dashboard/atomic_verify_fixture","reportingTimezone":"America/Phoenix","clickupListIds":[],"marginAliases":[],"configStatus":"approved","fixedValues":{"monthlyBudget":100,"monthlyHoursAllotment":10},"metrics":[{"key":"budget_pacing","label":"budget_pacing","adapterKey":"approved.budget_pacing","required":true,"weight":20,"direction":"lower_is_better","greenThreshold":10,"yellowThreshold":20,"sourceKeys":["paid"]},{"key":"hours","label":"hours","adapterKey":"approved.hours","required":true,"weight":20,"direction":"lower_is_better","greenThreshold":80,"yellowThreshold":100,"sourceKeys":["tasks"]},{"key":"margin","label":"margin","adapterKey":"approved.margin","required":true,"weight":20,"direction":"higher_is_better","greenThreshold":50,"yellowThreshold":25,"sourceKeys":["paid"]},{"key":"north_star","label":"north_star","adapterKey":"approved.north_star","required":true,"weight":20,"direction":"higher_is_better","greenThreshold":10,"yellowThreshold":5,"sourceKeys":["paid"]},{"key":"overdue_tasks","label":"overdue_tasks","adapterKey":"approved.overdue_tasks","required":true,"weight":20,"direction":"lower_is_better","greenThreshold":0,"yellowThreshold":1,"sourceKeys":["tasks"]}],"sources":[{"sourceKey":"failed","requestFingerprint":"cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc","permittedFactFields":[],"freshnessPolicy":{"maximumLagDays":0},"provider":"supabase","project":"eic","relation":"failed_facts"},{"sourceKey":"paid","requestFingerprint":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","permittedFactFields":["currentRows","fulfillmentCost","monthSpend","previousRows","revenue"],"freshnessPolicy":{"maximumLagDays":3},"provider":"supabase","project":"eic","relation":"budget_pacing_facts"},{"sourceKey":"tasks","requestFingerprint":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb","permittedFactFields":["hoursUsed","overdueTaskCount"],"freshnessPolicy":{"maximumLagDays":0},"provider":"clickup","endpointFamily":"team-time-entries-and-overdue-tasks","permitsTasks":true,"allowedListIds":["1"]}]}]}$revision$::jsonb;
  v_later_revision_id uuid := '644b550a-e768-8640-ba81-ebe148c3fdcc';
  v_later_revision_hash text := '644b550ae76896407a81ebe148c3fdcc0a84a19d4246053927628e7aff2f3a52';
...
begin
  ...
  foreach v_bad in array array[
    ...
    v_failed:=false; begin
      perform public.client_health_complete_source_run(v_source,v_run,'succeeded',v_base+interval '1 second',date '2026-08-20',1,
        ...
        jsonb_build_object('currentRows',jsonb_build_array(jsonb_build_object('spend',10,'results',2)),'fulfillmentCost',5,'monthSpend',50,'previousRows',jsonb_build_array(jsonb_build_object('spend',9,'results',1)),'revenue',20),
  ...
  foreach v_bad in array array[
    ...
    v_failed:=false; begin
      perform public.client_health_complete_source_run(v_source,v_run,
        ...
        case when v_bad->>'kind' in ('failed-with-count','oversized-error')
          then jsonb_build_object('currentRows',null,'fulfillmentCost',null,'monthSpend',null,'previousRows',null,'revenue',null)
          else jsonb_build_object('currentRows',jsonb_build_array(jsonb_build_object('spend',10,'results',2)),'fulfillmentCost',5,'monthSpend',50,'previousRows',jsonb_build_array(jsonb_build_object('spend',9,'results',1)),'revenue',20) end,
  ...
  perform public.client_health_complete_source_run(v_source,v_run,'succeeded',v_base+interval '1 second',date '2026-08-20',1,repeat('a',64),
    ...
    jsonb_build_object('currentRows',jsonb_build_array(jsonb_build_object('spend',10,'results',2)),'fulfillmentCost',5,'monthSpend',50,'previousRows',jsonb_build_array(jsonb_build_object('spend',9,'results',1)),'revenue',20),null,null,v_invocation,v_attempt,1);
  ...
  v_calculation:=public.client_health_calculate_snapshot(v_run,v_client,v_base+interval '1 second');
  v_snapshot:=(v_calculation->>'snapshotId')::uuid;
  v_snapshot_json:=v_calculation->'snapshot';
  -- Fixed cross-runtime parity vector generated by engine.ts for the exact revision,
  -- committed facts, and source statuses above. This is not derived from SQL output.
  if v_snapshot_json->>'currentWindowStart'<>'2026-08-07' or v_snapshot_json->>'previousWindowStart'<>'2026-07-24'
     or abs((v_snapshot_json->>'expectedSpend')::numeric-64.51612903225806::numeric)>0.000000000001
     or (v_snapshot_json->>'marginPercent')::numeric<>75
     or v_snapshot_json->>'overallStatus'<>'at_risk' or (v_snapshot_json->>'overallScore')::numeric<>60
     or v_snapshot_json->'reasons'<>jsonb_build_array('Budget pacing is 14.52 (watch).','North-star trend is -44.44 (at risk).','Hours utilization is 31 (healthy).','Overdue tasks is 1 (watch).','Margin is 75 (healthy).')
     or abs((v_snapshot_json->'dimensionStatuses'->'budget_pacing'->>'value')::numeric-14.516129032258064::numeric)>0.000000000001
     or abs((v_snapshot_json->'dimensionStatuses'->'north_star'->>'value')::numeric-(-44.44444444444444)::numeric)>0.000000000001
     or (v_snapshot_json->'dimensionStatuses'->'hours'->>'value')::numeric<>31
     or (v_snapshot_json->'dimensionStatuses'->'overdue_tasks'->>'value')::numeric<>1
     or (v_snapshot_json->'dimensionStatuses'->'margin'->>'value')::numeric<>75
     or ((v_snapshot_json->'dimensionStatuses'->'budget_pacing')-'value'::text)<>jsonb_build_object('status','watch','reason','Budget pacing is 14.52 (watch).','required',true,'weight',20)
     or ((v_snapshot_json->'dimensionStatuses'->'north_star')-'value'::text)<>jsonb_build_object('status','at_risk','reason','North-star trend is -44.44 (at risk).','required',true,'weight',20)
     or ((v_snapshot_json->'dimensionStatuses'->'hours')-'value'::text)<>jsonb_build_object('status','healthy','reason','Hours utilization is 31 (healthy).','required',true,'weight',20)
     or ((v_snapshot_json->'dimensionStatuses'->'overdue_tasks')-'value'::text)<>jsonb_build_object('status','watch','reason','Overdue tasks is 1 (watch).','required',true,'weight',20)
     or ((v_snapshot_json->'dimensionStatuses'->'margin')-'value'::text)<>jsonb_build_object('status','healthy','reason','Margin is 75 (healthy).','required',true,'weight',20) then
    raise exception 'VERIFY FAILED: SQL calculator diverged from fixed engine.ts parity vector';
  end if;
  update public.client_health_source_runs set facts=pg_catalog.jsonb_set(pg_catalog.jsonb_set(facts,'{revenue}','100000'::jsonb),'{fulfillmentCost}','98995'::jsonb) where id=v_source;
  if public.client_health_calculate_snapshot(v_run,v_client,null)->'snapshot'->'dimensionStatuses'->'margin'->>'reason'<>'Margin is 1.01 (at risk).' then
    raise exception 'VERIFY FAILED: SQL decimal-tie reason formatting diverged from engine.ts';
  end if;
  update public.client_health_source_runs set facts=pg_catalog.jsonb_set(pg_catalog.jsonb_set(facts,'{revenue}','20'::jsonb),'{fulfillmentCost}','5'::jsonb) where id=v_source;
  v_calculation:=public.client_health_calculate_snapshot(v_run,v_client,null); v_snapshot:=(v_calculation->>'snapshotId')::uuid; v_snapshot_json:=v_calculation->'snapshot';

  -- Caller calculation fields are untrusted previews. A forged all-healthy payload,
  -- changed values/reasons/source states, proof, and idempotency must never influence
  -- committed rows; this subtransaction proves DB replacement and then rolls back.
  v_tasks:=jsonb_build_array(jsonb_build_object('refreshRunId',v_run::text,'snapshotId',v_snapshot::text,'clickupTaskId','FORGED1','listId','1','taskName','Forged preview task','taskUrl','https://app.clickup.com/t/FORGED1','dueAt','2026-08-20T12:00:00.000Z','displayRank',1));
  v_bad:=pg_catalog.jsonb_set(v_snapshot_json,'{monthSpend}','999'::jsonb);
  v_bad:=pg_catalog.jsonb_set(v_bad,'{sourceStatuses,paid,status}','"failed"'::jsonb);
  v_bad:=pg_catalog.jsonb_set(v_bad,'{overallStatus}','"healthy"'::jsonb);
  v_bad:=pg_catalog.jsonb_set(v_bad,'{reasons}',jsonb_build_array('Forged all healthy.'));
  v_bad:=pg_catalog.jsonb_set(v_bad,'{evidenceHash}',to_jsonb(repeat('f',64)));
  v_bundle:=jsonb_build_object('configRevisionId',v_revision_id::text,'configRevisionHash',v_revision_hash,'idempotencyKey',repeat('f',64),'evidenceHash',repeat('f',64),'snapshotId',v_snapshot::text,'snapshot',v_bad,'tasks',v_tasks);
  v_failed:=false;
  begin
    v_receipt:=public.client_health_persist_snapshot_bundle(v_bundle,v_invocation,v_attempt,1);
    if v_receipt->>'snapshotId'<>v_calculation->>'snapshotId' or v_receipt->>'evidenceHash'<>v_calculation->>'proofHash'
       or not exists(select 1 from public.client_health_snapshots where id=(v_calculation->>'snapshotId')::uuid and month_spend=(v_calculation->'snapshot'->>'monthSpend')::numeric and overall_status=v_calculation->'snapshot'->>'overallStatus' and source_statuses=v_calculation->'snapshot'->'sourceStatuses') then
      raise exception 'VERIFY FAILED: caller-forged calculation influenced authoritative persistence';
    end if;
    raise exception 'EXPECTED_AUTHORITY_ROLLBACK';
  exception when others then
    if sqlerrm='EXPECTED_AUTHORITY_ROLLBACK' then v_failed:=true; else raise; end if;
  end;
  if not v_failed or exists(select 1 from public.client_health_snapshots where refresh_run_id=v_run) then raise exception 'VERIFY FAILED: authority proof subtransaction did not roll back'; end if;
  v_snapshot_json:=v_calculation->'snapshot';
  ...
  v_idempotency:=v_calculation->>'idempotencyKey'; v_bundle:=jsonb_build_object('configRevisionId',v_revision_id::text,'configRevisionHash',v_revision_hash,'idempotencyKey',v_idempotency,'evidenceHash',v_calculation->>'proofHash','snapshotId',v_snapshot::text,'snapshot',v_snapshot_json,'tasks',v_tasks);
  ...
  -- Preserve malformed timestamp, duplicate-ID, and rank checks below using an
  -- authoritative overdue count of two so no earlier count mismatch can mask them.
  update public.client_health_source_runs set facts=pg_catalog.jsonb_set(facts,'{overdueTaskCount}','2'::jsonb),evidence=pg_catalog.jsonb_set(evidence,'{overdueTaskCount}','2'::jsonb) where id=v_task_source;
  v_calculation:=public.client_health_calculate_snapshot(v_run,v_client,null); v_snapshot:=(v_calculation->>'snapshotId')::uuid; v_snapshot_json:=v_calculation->'snapshot';
  ...
  v_idempotency:=v_calculation->>'idempotencyKey';
  v_bundle:=jsonb_build_object('configRevisionId',v_revision_id::text,'configRevisionHash',v_revision_hash,'idempotencyKey',v_idempotency,'evidenceHash',v_calculation->>'proofHash','snapshotId',v_snapshot::text,'snapshot',v_snapshot_json,'tasks',v_tasks);
  v_failed:=false; begin perform public.client_health_persist_snapshot_bundle(v_bundle,v_invocation,v_attempt,1); exception when others then v_failed:=true; end;
  ...
  update public.client_health_source_runs set facts=pg_catalog.jsonb_set(facts,'{overdueTaskCount}','1'::jsonb),evidence=pg_catalog.jsonb_set(evidence,'{overdueTaskCount}','1'::jsonb) where id=v_task_source;
  v_calculation:=public.client_health_calculate_snapshot(v_run,v_client,null); v_snapshot:=(v_calculation->>'snapshotId')::uuid; v_snapshot_json:=v_calculation->'snapshot';
  v_tasks:=pg_catalog.jsonb_build_array(jsonb_build_object('refreshRunId',v_run::text,'snapshotId',v_snapshot::text,'clickupTaskId','TASK1','listId','1','taskName','Task one','taskUrl','https://app.clickup.com/t/TASK1','dueAt','2026-08-20T12:00:00.000Z','displayRank',1));
  v_idempotency:=v_calculation->>'idempotencyKey'; v_bundle:=jsonb_build_object('configRevisionId',v_revision_id::text,'configRevisionHash',v_revision_hash,'idempotencyKey',v_idempotency,'evidenceHash',v_calculation->>'proofHash','snapshotId',v_snapshot::text,'snapshot',v_snapshot_json,'tasks',v_tasks);
  ...
  v_failed:=false; begin perform public.client_health_validate_refresh_run(v_run,v_base+interval '3 seconds',repeat('f',64),v_invocation,v_attempt,1); exception when others then v_failed:=true; end;
  if not v_failed then raise exception 'VERIFY FAILED: caller-forged aggregate evidence hash was accepted'; end if;
  perform public.client_health_validate_refresh_run(v_run,v_base+interval '3 seconds',public.client_health_refresh_evidence_hash(v_run),v_invocation,v_attempt,1);
