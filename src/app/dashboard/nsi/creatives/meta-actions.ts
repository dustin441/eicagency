...
const DASHBOARD_PATH = '/dashboard/nsi/creatives';
...
export async function canEditNsiMetaCreativeTests() {
  const editor = await requireAgencyEditor();
  return !('error' in editor);
}

export async function setNsiMetaCreativeTestStatus(
  testId: number,
  ...
  if (fetchError || !test) return { ok: false, message: 'NSI Meta test not found.' };
