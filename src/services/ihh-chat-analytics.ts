...
export async function fetchIhheChatSummary(
  startDate?: string,
  endDate?: string,
  days?: number,
): Promise<IhheSummaryRow[]> {
  const supabase = createSpartacoSupabaseClient();
  const { start, end } = resolveDateRange(startDate, endDate, days);

  const { data, error } = await supabase.rpc('ihh_chat_summary', {
...
export async function fetchIhheChatCampaigns(
  startDate?: string,
  endDate?: string,
  days?: number,
  limit = 20,
): Promise<IhheCampaignRow[]> {
  const supabase = createSpartacoSupabaseClient();
  const { start, end } = resolveDateRange(startDate, endDate, days);

  const { data, error } = await supabase.rpc('ihh_chat_campaigns', {
...
export async function fetchIhheChatSpendTrend(
  startDate?: string,
  endDate?: string,
  days?: number,
  ...
  const { data, error } = await supabase.rpc('ihh_chat_spend_trend', {
    p_start: start, p_end: end,
  });
  if (error) throw new Error(error.message);
...
export async function fetchIhheChatMetaCreatives(
  startDate?: string,
  endDate?: string,
  days?: number,
  limit = 10,
  ...
  const { data, error } = await supabase.rpc('ihh_chat_meta_creatives', {
    p_start: start, p_end: end, p_limit: limit,
  });
  if (error) throw new Error(error.message);
