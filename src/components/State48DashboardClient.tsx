...
import type { State48DashboardData } from '@/services/state48-analytics';
...
function BudgetPacing({
  ...
  pacing: State48DashboardData['budgetPacing'];
  isAdmin: boolean;
  updateBudget: (n: number) => Promise<{ error?: string }>;
}) {
...
function TrendChart({ timeSeries }: { timeSeries: State48DashboardData['timeSeries'] }) {
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      ...
      <ResponsiveContainer width="100%" height={240}>
        <AreaChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="s48SpendGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor="#EB541E" stopOpacity={0.15} />
              <stop offset="95%" stopColor="#EB541E" stopOpacity={0} />
            </linearGradient>
            <linearGradient id="s48MetricGrad" x1="0" y1="0" x2="0" y2="1">
          ...
          <YAxis yAxisId="metric" orientation="right" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false} tickFormatter={v => isRoas ? Number(v).toFixed(2) + 'x' : isAov ? fmtAov(Number(v)) : fmtN(Number(v))} />
          <Tooltip
          ...
          <Area yAxisId="spend" type="monotone" dataKey="Spend" stroke="#EB541E" strokeWidth={2} fill="url(#s48SpendGrad)" dot={false} />
          <Area yAxisId="metric" type="monotone" dataKey={activeLabel} stroke="#0B4A31" strokeWidth={2} fill="url(#s48MetricGrad)" dot={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
function CampaignTable({ rows }: { rows: State48DashboardData['campaignRows'] }) {
  ...
  const cols: { key: CampSortKey; label: string; fmt: (v: number) => string; prevKey: keyof State48DashboardData['campaignRows'][0] }[] = [
...
export default function State48DashboardClient({
  data,
  isAdmin,
  updateBudget,
}: {
  data: State48DashboardData;
  isAdmin: boolean;
  updateBudget: (n: number) => Promise<{ error?: string }>;
}) {
    ...
    <div className="min-h-screen bg-gray-50/50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-1">Google Ads AOV</p>
              <p className="text-3xl font-bold text-gray-900">
                {summary.aov !== null ? fmtAov(summary.aov) : '—'}
              </p>
              <p className="mt-1 text-xs text-gray-400">Google Ads-attributed revenue ÷ Google Ads purchases</p>
            </div>
