...
function WeeklyExecutiveSummary({ readout }: { readout: ArabellasDashboardData['weeklyReadout'] }) {
    ...
    <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-gray-900">Arabella Hotels</h2>
...
function BudgetPacing({
  ...
  pacing: ArabellasDashboardData['budgetPacing'];
  isAdmin: boolean;
  updateBudget: (n: number) => Promise<{ error?: string }>;
}) {
...
function TrendChart({ timeSeries }: { timeSeries: ArabellasDashboardData['timeSeries'] }) {
  ...
  const isCurrency = activeMetric === 'roas';
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      ...
      <ResponsiveContainer width="100%" height={240}>
        <AreaChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="arSpendGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor="#EB541E" stopOpacity={0.15} />
              <stop offset="95%" stopColor="#EB541E" stopOpacity={0} />
            </linearGradient>
            <linearGradient id="arMetricGrad" x1="0" y1="0" x2="0" y2="1">
          ...
          <YAxis yAxisId="metric" orientation="right" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false} tickFormatter={v => isCurrency ? Number(v).toFixed(2) + 'x' : fmtN(Number(v))} />
          <Tooltip
          ...
          <Area yAxisId="spend" type="monotone" dataKey="Spend" stroke="#EB541E" strokeWidth={2} fill="url(#arSpendGrad)" dot={false} />
          <Area yAxisId="metric" type="monotone" dataKey={activeLabel} stroke="#0B4A31" strokeWidth={2} fill="url(#arMetricGrad)" dot={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
export default function ArabellaHotelsDashboardClient({
  data,
  isAdmin,
  updateBudget,
}: {
  data: ArabellasDashboardData;
  isAdmin: boolean;
  updateBudget: (n: number) => Promise<{ error?: string }>;
}) {
    ...
    <div className="min-h-screen bg-gray-50/50">
      <div className="bg-white border-b border-gray-100 px-6 py-5">
        <div className="max-w-7xl mx-auto flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-gray-900">Arabella Hotels</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <KpiCard label="ROAS"        value={summary.roas}        prev={prevSummary.roas}        format={fmtRoas} goal={10} goalFmt={fmtRoas} />
        </div>
